...
function connectFour(id, user) {
	...
	var margin = {
		top: 50,
		left: Math.max(w - boardW, 0) / 2,
		right: Math.max(w - boardW, 0) / 2,
		bottom: 50
	};
	...
	var ai = svg.selectAll('.ai').data(aiData).enter().append('rect').attr('width', function(d) {
			return d.w;
		})
	...
	var aiText = svg.selectAll('.aiText').data(aiData).enter().append('text').attr('class', 'aiText')
		.attr('transform', function(d) {
			return 'translate(' + (margin.left + d.x + d.w / 2) + ',' + (margin.top + d.y + d.h / 2 + 7.5) + ')';
		})
		.text('0');
